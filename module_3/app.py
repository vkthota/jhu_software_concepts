...
def index():
    ...
    answer_1 = result_1[0][0] if result_1 else "Query Failed"
    query_results.append(("1. How many applicants for Fall 2025?", f"{answer_1:,}"))
    ...
    answer_3_list = []
    if result_3 and result_3[0][0] is not None:
        avg_gpa, avg_gre, avg_gre_v, avg_gre_aw = result_3[0]
        answer_3_list.append(f"GPA: {avg_gpa:.2f}" if avg_gpa else "GPA: N/A")
        answer_3_list.append(f"GRE Total: {avg_gre:.2f}" if avg_gre else "GRE Total: N/A")
        answer_3_list.append(f"GRE Verbal: {avg_gre_v:.2f}" if avg_gre_v else "GRE Verbal: N/A")
        answer_3_list.append(f"GRE AW/Quant: {avg_gre_aw:.2f}" if avg_gre_aw else "GRE AW/Quant: N/A")
    query_results.append(("3. Average Scores (All Terms):", " | ".join(answer_3_list) if answer_3_list else "No Data"))

    # --- Query 4 ---
    query_4 = "SELECT AVG(gpa) FROM applicants WHERE us_or_international = 'American or Other' AND term = 'Fall 2025';"
    ...
    # --- Query 5 ---
    query_5 = "SELECT 100.0 * COUNT(*) FILTER (WHERE status = 'Acceptance') / NULLIF(COUNT(*), 0) FROM applicants WHERE term = 'Fall 2025';"
    ...
    # --- Query 6 ---
    query_6 = "SELECT AVG(gpa) FROM applicants WHERE term = 'Fall 2025' AND status = 'Acceptance';"
    ...
    # --- Query 7 ---
    query_7 = "SELECT COUNT(*) FROM applicants WHERE program ILIKE '%JHU%' AND program ILIKE '%Computer Science%' AND degree = 'Masters';"
    result_7 = execute_query(query_7)
    answer_7 = result_7[0][0] if result_7 else "Query Failed"
    query_results.append(("7. JHU Masters in Computer Science Applicants:", answer_7))
